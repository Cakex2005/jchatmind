...
import { Alert, Button, Card, Space, Typography, Select } from "antd";
    ...
    <div className="flex flex-col h-full">
      ...
      {(agentsError || (!agentsLoading && agents.length === 0)) && (
        <div className="px-4 pt-3">
          <Alert
            type={agentsError ? "error" : "warning"}
            showIcon
            message={
              agentsError
                ? `智能体列表加载失败：${agentsError}`
                : "还没有可用的智能体助手"
            }
            description={
              agentsError
                ? "后端可能未启动，或浏览器无法访问 http://localhost:8080（代理/插件常拦截 localhost）。"
                : "请先在「智能体」中创建一个助手，再回来发送消息。"
            }
            action={
              onRetryAgents ? (
